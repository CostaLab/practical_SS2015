...
def get_pvalue(forward_match, reverse_match, forward_mismatch, reverse_mismatch):
    ...
    if test == "fisher" and False: #barni we don't compute fisher, for testing purposes
        p_value = pvalue(forward_match, forward_mismatch, reverse_match, reverse_mismatch)
        p_value = min(p_value.left_tail, p_value.right_tail, p_value.two_tail)
    else:
    ...
    #barni TODO remove
    #print fisher and chi result for same data
    fisher_pvalue = pvalue(forward_match, forward_mismatch, reverse_match, reverse_mismatch) #returns (left_t, right_t, two_t)
    if fisher_pvalue.left_tail != p_value or fisher_pvalue.right_tail != p_value or fisher_pvalue.two_tail != p_value:
        print("fisher[left, right, two] : ", str(fisher_pvalue.left_tail), str(fisher_pvalue.right_tail), str(fisher_pvalue.two_tail))
        print("chi output : ", str(p_value))
    return p_value
