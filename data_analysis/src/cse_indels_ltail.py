...
def get_pvalue(fm, rm, fmm, rmm):
    ...
    #fabio setting left-tailed fisher
    #p_value_tmp = f(table)[0]
    p_value_tmp = f(table,alternative="l")[0]
    #/fabio
...
def get_annotate_genome(genome, bampath, learn_chrom):
    ...
    len_genome = len(genome)
    #initialize
    fm = array.array('H',[0] * len_genome)
    fmm = array.array('H',[0] * len_genome)
    rm = array.array('H',[0] * len_genome)
    rmm = array.array('H',[0] * len_genome)
    #insertions
    f_ins_m = array.array('h',[0] * len_genome)  # insertion to the ref
    f_ins_mm = array.array('h',[0] * len_genome) # no insertion to the ref
    r_ins_m = array.array('h',[0] * len_genome)
    r_ins_mm = array.array('h',[0] * len_genome)
    #deletions
    f_del_m = array.array('H',[0] * len_genome)
