...
def log(result, s):
    ...
    print("#Sequence", "Occurrence", "Forward Match", "Backward Match", 
          "Forward Mismatch", "Backward Mismatch", "Strand Bias Score", sep = '\t')
    if s == "deletions":
        with open(LOG_DEL) as f:
            seq = result[0]
            print(seq,count(seq,genome),str(result[1]),str(result[2]),
                    str(result[3]),str(result[4]),str(result[5]),sep = '\t')
