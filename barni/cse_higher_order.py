...
import time
DEBUG_INDEL = True
LOG_DEL     = "../output/bsubtilis/del_1.log"
...
def get_annotate_qgram(genome, genome_annotate, q, n_order):
    ...
    for i in range(len(genome) - q):
        ...
        lim = min(len_genome, q + n_order + 1)
        for x in range(q, lim):
        ...
        if qgram_first.has_key(qgram):
            ...
            #append list of dummy zeros (fm,rm,...) for next iterations
            while len(qgram_first[qgram]) < n_order + 1:
...
def ident(genome, genome_annotate, q, n, alpha=0.05, epsilon=0.03, delta=0.05, n_order=0):
    ...
    qgram_annotate, qgram_ins_annotate, qgram_del_annotate = get_annotate_qgram(
            genome, genome_annotate, q, n_order) #annotate each q-gram with Strand Bias Table
